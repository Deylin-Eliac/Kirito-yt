...
app.get("/ytmp3", async (req, res) => {
  ...
  if (!query) return res.status(400).json({ status: false, error: "No URL provided" });

  try {
    const url = savetube.isUrl(query)
      ? query
      : (await yts.search({ query, pages: 1 })).videos[0]?.url;
